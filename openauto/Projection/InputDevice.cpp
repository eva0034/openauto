...
bool InputDevice::handleTouchEvent(QEvent* event) {
  ...
  switch (event->type()) {
    ...
    case QEvent::TouchUpdate:
      if (qtTouchEvent->touchPointStates().testFlag(Qt::TouchPointPressed))
        type = aasdk::proto::enums::TouchAction::POINTER_DOWN;
      else if (qtTouchEvent->touchPointStates().testFlag(
  ...
  for (int i = 0; i < pointers.count(); i++) {
    ...
    touchLocation->set_x((static_cast<float>(pointers[i].pos().x()) /
                          touchscreenGeometry_.width()) *
                         displayGeometry_.width());
    touchLocation->set_y((static_cast<float>(pointers[i].pos().y()) /
                          touchscreenGeometry_.height()) *
