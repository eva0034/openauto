...
DestRect::DestRect() : fullscreen(OMX_TRUE) {}

DestRect::DestRect(OMX_S16 _xOffset, OMX_S16 _yOffset, OMX_S16 _width,
                   OMX_S16 _height)
...
OMXVideoOutput::OMXVideoOutput(
    configuration::IConfiguration::Pointer configuration, DestRect destRect,
...
bool OMXVideoOutput::setupDisplayRegion() {
  ...
  if (displayRegion.fullscreen == OMX_FALSE) {
    ...
    displayRegion.set = static_cast<OMX_DISPLAYSETTYPE>(
        displayRegion.set | OMX_DISPLAY_SET_ALPHA | OMX_DISPLAY_SET_DEST_RECT);
  }
...
bool OMXVideoOutput::createComponents() {
  if (ilclient_create_component(client_, &components_[VideoComponent::DECODER],
  ...
  if (ilclient_create_component(client_,
                                &components_[VideoComponent::SCHEDULER],
                                const_cast<char*>("video_scheduler"),
                                ILCLIENT_DISABLE_ALL_PORTS) != 0) {
    OPENAUTO_LOG(error)
