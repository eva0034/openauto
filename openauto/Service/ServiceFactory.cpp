...
ServiceFactory::ServiceFactory(
    ...
    configuration::IConfiguration::Pointer configuration, QWidget* activeArea,
    std::function<void(bool)> activeCallback, bool nightMode)
...
#if defined USE_OMX
      ,
      omxVideoOutput_(std::make_shared<projection::OMXVideoOutput>(
          configuration_, this->QRectToDestRect(screenGeometry_),
          activeCallback_))
#elif defined USE_GST
      ,
      gstVideoOutput_((QGst::init(nullptr, nullptr),
                       std::make_shared<projection::GSTVideoOutput>(
                           configuration_, activeArea_, activeCallback_)))
#else
      ,
...
ServiceList ServiceFactory::create(
  ...
  std::shared_ptr<SensorService> sensorService =
      std::make_shared<SensorService>(ioService_, messenger, nightMode_);
  sensorService_ = sensorService;
  serviceList.emplace_back(sensorService);

  serviceList.emplace_back(this->createVideoService(messenger));
  serviceList.emplace_back(this->createBluetoothService(messenger));
  std::shared_ptr<NavigationStatusService> navStatusService =
      this->createNavigationStatusService(messenger);
  navStatusService_ = navStatusService;
  serviceList.emplace_back(navStatusService);
  std::shared_ptr<MediaStatusService> mediaStatusService =
      this->createMediaStatusService(messenger);
  ...
  std::shared_ptr<InputService> inputService =
      this->createInputService(messenger);
...
IService::Pointer ServiceFactory::createVideoService(
  ...
  if (activeCallback_ != nullptr) {
    QObject::connect(qtVideoOutput_, &projection::QtVideoOutput::startPlayback,
                     [callback = activeCallback_]() { callback(true); });
    QObject::connect(qtVideoOutput_, &projection::QtVideoOutput::stopPlayback,
                     [this]() {
  ...
  projection::IVideoOutput::Pointer videoOutput(
      qtVideoOutput_, std::bind(&QObject::deleteLater, std::placeholders::_1));
#endif
...
std::shared_ptr<NavigationStatusService>
ServiceFactory::createNavigationStatusService(
    aasdk::messenger::IMessenger::Pointer messenger) {
  return std::make_shared<NavigationStatusService>(ioService_, messenger,
                                                   aa_interface_);
}

std::shared_ptr<MediaStatusService> ServiceFactory::createMediaStatusService(
    aasdk::messenger::IMessenger::Pointer messenger) {
  return std::make_shared<MediaStatusService>(ioService_, messenger,
                                              aa_interface_);
}

std::shared_ptr<InputService> ServiceFactory::createInputService(
    aasdk::messenger::IMessenger::Pointer messenger) {
  QRect videoGeometry;
  ...
  videoGeometry.setWidth(videoGeometry.width() -
                         configuration_->getVideoMargins().width());
  videoGeometry.setHeight(videoGeometry.height() -
                          configuration_->getVideoMargins().height());

  QObject* inputObject = activeArea_ == nullptr
                             ? qobject_cast<QObject*>(QApplication::instance())
                             : qobject_cast<QObject*>(activeArea_);
  inputDevice_ = std::make_shared<projection::InputDevice>(
      *inputObject, configuration_, std::move(screenGeometry_),
      std::move(videoGeometry));

  return std::make_shared<InputService>(
      ioService_, messenger,
      std::move(projection::IInputDevice::Pointer(inputDevice_)));
}
...
void ServiceFactory::setAndroidAutoInterface(
    IAndroidAutoInterface* aa_interface) {
  if (aa_interface == NULL)
    return;
  this->aa_interface_ = aa_interface;

  if (std::shared_ptr<MediaStatusService> mediaStatusService =
          mediaStatusService_.lock()) {
    mediaStatusService->setAndroidAutoInterface(aa_interface);
  }
  if (std::shared_ptr<NavigationStatusService> navStatusService =
          navStatusService_.lock()) {
...
void ServiceFactory::sendButtonPress(
