...
class IAndroidAutoInterface
    : public std::enable_shared_from_this<IAndroidAutoInterface> {
 public:
  std::shared_ptr<IAndroidAutoInterface> getPtr() { return shared_from_this(); }
  typedef std::shared_ptr<IAndroidAutoInterface> Pointer;

  virtual ~IAndroidAutoInterface() = default;

  virtual void mediaPlaybackUpdate(
      const aasdk::proto::messages::MediaInfoChannelPlaybackData& playback) = 0;
  virtual void mediaMetadataUpdate(
      const aasdk::proto::messages::MediaInfoChannelMetadataData& metadata) = 0;
  virtual void navigationStatusUpdate(
      const aasdk::proto::messages::NavigationStatus& navStatus) = 0;
  virtual void navigationTurnEvent(
      const aasdk::proto::messages::NavigationTurnEvent& turnEvent) = 0;
  virtual void navigationDistanceEvent(
      const aasdk::proto::messages::NavigationDistanceEvent& distanceEvent) = 0;
  void setServiceFactory(ServiceFactory* serviceFactory) {
    this->m_serviceFactory = serviceFactory;
  }
  void injectButtonPress(aasdk::proto::enums::ButtonCode::Enum buttonCode,
  ...
  void injectButtonPress(aasdk::proto::enums::ButtonCode::Enum buttonCode,
                         projection::ButtonEventType buttonEventType) {
    this->injectButtonPress(buttonCode, projection::WheelDirection::NONE,
                            buttonEventType);
  }
